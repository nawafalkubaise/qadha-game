...
const BI={
  ...
  normal:"عادي 😊 · Normal 😊",
  hard:"صعب 🔥 · Hard 🔥",
  seenjeemHint:"قدها؟: دمج أسئلة عربية وإنجليزية واحتياطية؛ عالمي منفصل: عربي أو إنجليزي فقط. · Qadha?: merges AR/EN + fallbacks; pick Global AR or Global EN for one language.",
...
export default function Qadha(){
  ...
  function CatIcon({cat,sz=22,r=6,border}){
    ...
    return <CategoryArt id={cat.id} color={cat.c} size={sz} radius={r} />;
  }
...
`}</style>
      ...
      {sc==="menu"&&<div style={W}><div style={P}>
        ...
        <button className="bg" style={{width:"100%",padding:"clamp(20px,4.5vw,26px)",fontSize:"clamp(18px,4vw,22px)",animation:"gl 2s ease-in-out infinite",marginBottom:14,borderRadius:18}} onClick={()=>{sfx.click();setSelCats([]);go("cats")}}>{tx.start}</button>
      ...
      {sc==="cats"&&<div style={WScroll}><div className="catsBody" style={{...PwideScroll,maxWidth:"min(920px,100%)"}}>
                    ...
                    return(
                      <div key={cat.id} role="button" tabIndex={selCats.length===8&&!sel?-1:0} className={`catsCard${sel?" catsCardSel":""}${selCats.length===8&&!sel?" catsCardLocked":""}`} onPointerDown={()=>togCat(cat)} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();togCat(cat)}}}>
                        ...
                        <div className="catsCardBody">
                          {cat.sjCover?<img src={cat.sjCover} alt="" referrerPolicy="no-referrer" style={{width:68,height:68,borderRadius:14,objectFit:"cover"}}/>:<CategoryArt id={cat.id} color={cat.c} size={68} radius={14}/>}
                        </div>
        ...
        <div className="catsChipRow">{selCats.map(cat=><button key={cat.id} type="button" className={`catsChip${selSet.has(cat.id)?" catsChipOn":""}`} onClick={()=>togCat(cat)}>{cat.sjCover?<img src={cat.sjCover} alt="" style={{width:52,height:52,borderRadius:12,objectFit:"cover"}}/>:<CategoryArt id={cat.id} color={cat.c} size={52} radius={12}/>}</button>)}{Array.from({length:8-selCats.length}).map((_,i)=><div key={`e${i}`} className="catsChipSlot"/>)}
        </div>
