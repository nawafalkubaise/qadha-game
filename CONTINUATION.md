...
| **الكود والتغييرات** | Git على فرع **`master`** — آخر commits تشمل `c908b74` (ميزات اللعبة) و`4ab7844` (توثيق). للقائمة: `git log --oneline -10` |
| **حالة العمل حالياً** | `git status` → يفضّل أن يظهر *working tree clean* بعد كل حفظ |
| **تشغيل اللعبة** | `npm install` ثم `npm run dev` → المتصفح: `http://localhost:5173` أو `http://localhost:5174` |
...
| `src/categoryArt.jsx` | رسم SVG مميز لكل فئة (`CategoryArt`) — يُعرض بدل الإيموجي في البطاقات والشرائح |
...
| `README.md` | إن وُجد |
| **`src/`** بالكامل | `Qadha.jsx`, `categoryArt.jsx`, `main.jsx`, `index.css`, `seenjeem.js`, `triviaFallbacks.js`, `triviaFallbacksEn.js` |
...
- **عالمي منفصل عربي/إنجليزي**؛ **إزالة الرهان**؛ **تصفير الفئات** عند العودة من القائمة/زر فئات جديدة؛ **`categoryArt.jsx`**.

**آخر تحديث للملف:** ٣ أبريل ٢٠٢٦ — إضافة «نقطة الرجوع» وتوحيد الوصف مع الكود الحالي.
...
**أهم commit للميزات:** `c908b74`. **آخر commit للتوثيق (حتى ٣ أبريل ٢٠٢٦):** `4ab7844`. للقائمة الكاملة: `git log --oneline -12`.
